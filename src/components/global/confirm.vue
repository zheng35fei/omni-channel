<template>
    ...
    >
        ...
        <div style="margin-top:-20px;margin-left:10px;">
            ...
            <p
                style="margin-left:42px;margin-top:-18px;color:#909399;font-size:12px;"
            >【警告】该操作不可恢复，请谨慎操作！</p>
        </div>
...
export default {
    ...
    props: ["mode", "content", "sucessMsg", "apiType", "method", "cbApiType"],
    methods: {
        confirm(url, param) {
            this.url = url;
            this.param = param;
            ...
            switch (this.mode) {
                ...
                case "delete":
                    _this.common.listDelete(_this, {
                        url: _this.url,
                        params: _this.param,
                        reqMethod: this.apiType,
                        method: this.method,
                        callback: res => {
                            if (res.success) {
                                // this.$store.dispatch("getList", this.cbApiType);
